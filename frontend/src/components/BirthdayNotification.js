            ...
            {displayBirthdays.map((birthday, index) => (
              <div key={birthday.staffId}>
                ...
                <div style={styles.birthdayItem}
                  onClick={e => {
                    e.stopPropagation();
                    // Only navigate to DM if it's someone else's birthday (not your own)
                    if (!birthday.isCurrentUser && birthday.staffId) {
                      navigate(`/chat?dm=${birthday.staffId}`);
                      setExpanded(false);
                    }
                  }}
                  title={!birthday.isCurrentUser ? "Click to send a birthday message" : ""}
                  style={{ ...styles.birthdayItem, cursor: birthday.isCurrentUser ? "default" : "pointer" }}>
